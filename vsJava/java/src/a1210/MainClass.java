...
//부모클래스(StarCraft)
//공통메소드 life를 정의. 자식 클래스를 오버라이드 함.
//자식클래스 life 메소드를 오버라이딩 하여 고유한 동작을 정의.
//canon, run, flt와 같은 각 클래스만의 고유 메소드 추가.

//메인클래스
//부모클래스 배열 StarCraft[] 를 생성하여 다양한 자식 클래스를 저장.
//다형성을 활용해 life메소드를 호출하여 자식 클래스의 오버라이드된 메소드를 실행.
//instanceof와 형변환을 사용하여 각 자식 클래스의 고유 메소드를 호출.
